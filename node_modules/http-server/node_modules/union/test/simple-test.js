...
var examplesDir = path.join(__dirname, '..', 'examples', 'simple'),
    ...
    fooURI = 'http://http://192.168.0.103:9090/foo',
    server;
  ...
  "When using union": {
    "a simple http server": {
      ...
      "a GET request to `/redirect`": {
        topic: function () {
          request.get({
            url: 'http://http://192.168.0.103:9090/redirect',
      ...
      "a GET request to `/custom_redirect`": {
        topic: function () {
          request.get({
            url: 'http://http://192.168.0.103:9090/custom_redirect',
            followRedirect: false
          }, this.callback);
        },
        "it should redirect to `/foo`": function (err, res, body) {
          assert.equal(res.statusCode, 301);
          assert.equal(res.headers.location, "http://http://192.168.0.103:9090/foo");
        }
      },
      "a GET request to `/async`": {
        topic: function () {
          request.get({
            url: 'http://http://192.168.0.103:9090/async',
            timeout: 500
          }, this.callback);
        },
